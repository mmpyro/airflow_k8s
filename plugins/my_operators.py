...
class MyFirstOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        log.info(f"Operator param {self.operator_param}")
        task_instance = context['task_instance']
        sensors_minute = task_instance.xcom_pull('my_sensor_task', key='sensors_minute')
        log.info(f'Valid minute as determined by sensor: {sensors_minute}')
...
class MyFirstSensor(BaseSensorOperator):
    ...
    def poke(self, context):
        ...
        if current_minute % 3 != 0:
            ...
            return False
        log.info(f"Current minute {current_minute} is divisible by 3, sensor will retry.")
        task_instance = context['task_instance']
